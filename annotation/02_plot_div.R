...
setwd("/data/users/jrotzetter/assembly_annotation_course/annotation/w6_TE_dynamics/TE_dating")

## Import the modified output from parseRM.pl ending with "landscape.Div.Rname.tab
dat <- fread("flye_polished.fasta.mod.out.landscape.Div.Rname.tab")
...
p1 <- ggplot(rep_table.m, aes(fill = Superfamily, x = age, weight = value / 1000000)) +
    ...
    # geom_vline( # has nothing to do with our current work
...
ggsave(filename = "TE-myr.png", width = 10, height = 5)
